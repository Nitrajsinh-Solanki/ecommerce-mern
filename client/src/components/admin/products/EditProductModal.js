...
const EditProductModal = () => {
  ...
  const [editFormData, setEditFormData] = useState({
    ...
    pPrice: "",
    pOffer: "",
    pAttributes: {}, // Initialize pAttributes here
    error: false,
    success: false,
  });
  ...
  useEffect(() => {
    if (data.editProductModal.modal) {
      setEditFormData({
        ...
        pAttributes: data.editProductModal.pAttributes, // Set existing attributes
      });
    }
  }, [data.editProductModal, categoryList]);
  ...
  const submitForm = async (e) => {
    e.preventDefault();
    if (!editFormData.pEditImages || editFormData.pEditImages.length < 2) {
      alert("Please upload at least 2 images", "red");
      return;
    }
    try {
        ...
        <div className="mt-32 md:mt-0 relative bg-white w-11/12 md:w-3/6 shadow-lg flex flex-col items-center space-y-4 px-4 py-4 md:px-8">
          ...
          <form className="w-full" onSubmit={(e) => submitForm(e)}>
                ...
                <select
                  onChange={(e) =>
                    setEditFormData({
                      ...
                      error: false,
                      success: false,
                      pCategory: { _id: e.target.value, cName: e.target.selectedOptions[0].text },
                    })
                  }
                  name="status"
                  className="px-4 py-2 border focus:outline-none"
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              {Object.keys(editFormData.pAttributes).map((key) => (
                <div key={key} className="flex flex-col">
                  ...
                  <input
                    type="text"
                    id={key}
                    value={editFormData.pAttributes[key] || ""}
                    onChange={(e) => {
                      setEditFormData({
                        ...editFormData,
                        pAttributes: {
                          ...editFormData.pAttributes,
                          [key]: e.target.value
                        }
                      });
                    }}
